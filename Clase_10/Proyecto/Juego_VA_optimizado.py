# ============================================================
# Juego_VA_optimizado.py — versión optimizada de Juego_VA.py
#
# Único cambio: overlay_rect() mezcla solo la región del rectángulo
# (ROI) en lugar de copiar y mezclar el frame completo. Es la
# optimización de "localidad" del laboratorio de jerarquía de memoria
# (Clase_10) aplicada al juego. El resto del juego es idéntico.
# ============================================================
import random
import time
...
def overlay_rect(frame, x1, y1, x2, y2, color, alpha=0.45):
    """Rectángulo semitransparente mezclando SOLO la región (ROI).

    La versión original copiaba el frame completo (1920x1080x3 = 6.2 MB) y
    mezclaba el frame completo con cv2.addWeighted para pintar un panel de
    unos cientos de píxeles: dos pasadas por toda la memoria, entre 6 y 8
    veces por frame. Fuera del rectángulo la copia es idéntica al frame,
    así que la mezcla no cambiaba nada: era trabajo de memoria desperdiciado.

    Aquí se recorta la región, se construye una capa del color del tamaño
    del rectángulo y se mezcla solo eso. El resultado es idéntico píxel a
    píxel (ver medir_overlay.py y ../benchmark_juego.c, fase 5).
    """
    h, w = frame.shape[:2]
    x1, x2 = sorted((x1, x2))
    y1, y2 = sorted((y1, y2))
    # cv2.rectangle con grosor -1 pinta incluyendo la esquina (x2, y2)
